...
- [ ] **Step 1: Write failing tests for normalizer + edit distance**
...
- [ ] **Step 3: Implement minimal normalizer, edit distance, and Search config fields**
...
- [ ] **Step 4: Extend `FoodPropertiesBindingTest` for new defaults**
...
- [ ] **Step 2: Run — expect FAIL; implement scorer; run PASS; commit**
...
- [ ] **Step 1: Add V5 migration file**
- [ ] **Step 2: Implement `ProductCandidateSearcher`**
...
- [ ] **Step 3: Keep deprecated `searchByDocument(String, Pageable)` delegating to tokenized path for any remaining callers, or replace call sites and delete LIKE-alphabetical query.**
...
- [ ] **Step 4: Unit-test candidate searcher with H2 Spring test or pure EntityManager test; commit**
...
- [ ] **Step 1: Write failing integration tests in `ProductControllerTest` (H2)**
...
- [ ] **Step 2: Implement service wiring; keep existing `searchReturnsLocalProducts` green**
- [ ] **Step 3: Optional unit test that OFF hits are re-scored above weak local** (mock OffClient returning a strong name; weak local distractor already in DB)
- [ ] **Step 4: `mvn test`; commit; push**
...
- [ ] **Step 1: Adjust `duplicateWarnings` to use normalizer + scorer**
- [ ] **Step 2: Add a focused test: submitting name `nutela` warns about existing Nutella when force is false / warnings returned**
- [ ] **Step 3: `mvn test`; commit; push**
...
- [ ] **Step 1: Update architecture §5.3** so it no longer implies FTS is implemented while code used LIKE
...
- [ ] **Step 3: Manual / API verification notes (for implementer)**
