...
- [ ] **Step 2: Fix compile breaks in tests constructing `ProductResponse`**

`RedisProductCacheTest.sampleProduct()` — insert `null` density for Nutella `"400 g"` (or assert null). Prefer explicit `null`.

- [ ] **Step 3: Add/extend a controller or mapper-focused assertion**
...
- [ ] **Step 3: Implement helper + extend `Product` type**
...
- [ ] **Step 4: Run helper tests PASS; commit**
...
- [ ] **Step 1: Write failing UI tests in `ProductPage.test.tsx`**
...
- [ ] **Step 2: Run ProductPage tests — expect FAIL**

```bash
cd frontend && npm test -- --run src/pages/ProductPage.test.tsx
```

- [ ] **Step 3: Implement UI changes in `ProductPage.tsx`**
...
- [ ] **Step 5: Final commit if doc-only deltas remain; push branch; open/update PR**
